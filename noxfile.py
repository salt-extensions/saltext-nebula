...
SALT_REQUIREMENT = os.environ.get("SALT_REQUIREMENT") or "salt>=3007"
...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/nebula/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/nebula/*")
            ...
            #    "--include=src/saltext/nebula/*,tests/*"
            # )
        finally:
